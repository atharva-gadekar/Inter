...
export default function Notif() {
  ...
  const handleConnectClick = (connectionId, notification) => {
    axios
			.patch(
			...
			.then((response) => {
				...
				if (index > -1) {
					...
					message.success("Added Connection Successfully");
				}
			})
			.catch((error) => {
				console.error(error);
    ...
    const handleIgnoreClick = (connectionId, notification) => {
			axios
				.patch(
					`https://inter-api-8q0x.onrender.com/user/${userId}/ignore/${connectionId}`,
					{},
					{
						headers: {
							Authorization: `Bearer ${token}`,
					...
					<div className="bg-white rounded-3xl">
						...
						<div className="flex justify-end items-center space-x-6 mt-4 pr-2">
							<button
								...
								onClick={() => handleConnectClick(notification.sender._id, notification)}
							>
								Connect
							</button>
							<button
								className="py-2 text-slate-500 font-normal"
								onClick={() => handleIgnoreClick(notification.sender._id, notification)}
							>
								Ignore
							</button>
						</div>
